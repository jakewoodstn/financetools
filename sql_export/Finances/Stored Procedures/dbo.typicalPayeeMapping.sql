-- Exported: 2026-06-21T21:55:41.440704+00:00
-- Schema:   dbo
-- Object:   typicalPayeeMapping
-- Type:     SQL_STORED_PROCEDURE
-- Created:  2016-03-06 07:39:26.873000
-- Modified: 2022-09-12 02:58:53.670000



CREATE PROCEDURE dbo.typicalPayeeMapping (@testid BIGINT = 0, @debug INT = 0)
AS
BEGIN
  ...
  INSERT INTO @workSurface
    ...
    CROSS APPLY STRING_SPLIT(REPLACE(description, ':', ' : '), ' ') s
  ...
  INSERT INTO @reference
    ...
    FROM (SELECT
       ...
       ,COALESCE(t.bankOrigDescription, t.origDescription, t.description) description
      FROM bankTransaction t
      WHERE COALESCE(t.bankOrigDescription, t.origDescription, '') <> t.description
      AND transactionDate >= DATEADD(YEAR, -3, GETDATE())) sq
    CROSS APPLY STRING_SPLIT(REPLACE(description, ':', ' : '), ' ') s
  ...
  INSERT INTO @results
    ...
    FROM (SELECT
       ...
       ,SUM(CASE
          WHEN testToken = matchToken THEN 1
          ELSE 0
        END) matches
  ...
  BEGIN
    ...
    SELECT
      x.*
     ,t1.origDescription wOrigDescription
     ,t1.description wDescription
     ,t2.origDescription rOrigDescription
     ,t2.description rDescription
    FROM (SELECT
        w.transactionId transIdw
       ,r.transactionId transIdr
      FROM (SELECT
    ...
    FROM @results
    INNER JOIN bankTransaction t1
      ON transIdw = t1.transactionId
    INNER JOIN bankTransaction t2
      ON transidr = t2.transactionId
